	...
	return (
		<article  className="my-10 w-full">
			...
			<main  className="place-items-center gap-4 grid grid-cols-2 m-auto py-2 w-full">
				...
				{itemsToRender?.map((a,i)=><p key={i}  onClick={()=>{setStep(i) 
					window.scroll(0, 1600)}} className={`duration-200 cursor-pointer  p-1 px-2 rounded-md font-bold text-white ${step == i? "bg-gray-500/40":"bg-gray-500/20"}`}>{i}</p>)}
					</div>
		</article>
	);
}
