...
function ProyectComp({
	...
	return (
		<article className="group flex flex-col items-center md:gap-4 lg:m-2 my-10 p-2 rounded-md w-[18rem]">
			<header className="bg-slate-500/30 rounded-md w-full md:w-[18rem] h-auto md:h-48 overflow-hidden">
				<img
					src={url}
					alt={`${title} image`}
			...
			<main className="flex flex-col justify-between gap-2 w-full">
				...
				<div className="flex self-start gap-2 py-2">
					{github ? (
						<a
						href={github}
						className="flex justify-center gap-1 bg-indigo-900/40 hover:bg-indigo-900 hover:shadow-[0px_0px_8px_4px_rgba(36,48,137,1)] px-4 py-1 rounded-md font-semibold text-white hover:scale-105 duration-150"
						>
...
{/* <div>
	...
	{techs.map((element, index) => {
		return (
			<li
			key={index}
			className="md:m-1 px-1 rounded-sm outline outline-1 font-semibold text-[#bbb] hover:text-white text-sm text-center duration-200 select-none"
			>
			{element}
				</li>
			);
